...
export function useFindUsersReducer(search, description) {
    ...
    useEffect(() => {
        ...
        async function listUsers() {
            try {
                ...
                const apiUrl = search ?
                    ` https://fakerestapi.azurewebsites.net/api/v1/Books/${search}` :
        ...
        async function listDescription() {
            try {
                // Altera a apiUrl de acordo com a string 'search'
                const apiUrl = description ?
                    ` https://fakerestapi.azurewebsites.net/api/v1/Books/${description}` :
                    ' https://fakerestapi.azurewebsites.net/api/v1/Books';
                const result = await axios.get(apiUrl);
                // Atualiza o estado de 'data' com o resultado da API
                if (result.data) {
